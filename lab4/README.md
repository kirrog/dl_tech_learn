...
Архитектура BERT (Bidirectional Encoder Representations from Transformers) взяла в себя только Encoder части из Transformer [3]. Это стек из 12 энкодеров в базовой и 24 в большой версии BERT (Рисунок 1) [4].

| ![](metrics/transformer.png) | ![](metrics/encoder.png) |
| --- | --- |

**Рисунок 1.** Архитектура BERT

Каждый энкодер такой же как в оригинальной статье про трансформер. Состоит из многоголового внимания, полносвязного слоя, нормализаций и скип-коннекшенов (Рисунок 2). В отличии от оригинального трансформера в BERT(base) количество энкодеров 12, размерность скрытого состояния 768, голов самовнимания у каждого экодера 12 и в общей сумме 110 млн. параметров.

![](metrics/encoder_arch.png)

**Рисунок 2.** Блок энкодера в Transformer
...
$$p(s_1 \dots s_m|x_1 \dots x_m) = p(s|x),$$

где $x$ обозначает входную последовательность $x_1 \dots x_m$, а $s$ - последовательность состояний $s_1 \dots s_m$.
...
$$p(s|x; w) = \frac{\exp(w \cdot \Phi(x, s))}{\sum_{s' \in S^m} \exp(w \cdot \Phi(x, s')} ,$$

где $S^m$ - множество всех возможных последовательностей меток токенов длины m.

Определение $\Phi(x, s)$ таково
...
$$\Phi(x, s) = \sum​ \log \phi_{EMIT}​(y_i \rightarrow x_i​) + \log \phi_{TRANS}​(y_{i-1}​ \rightarrow y_i​)$$
...
Задача декодирования в CRFs состоит в следующем: для заданной входной последовательности $x = x_1, x_2, \dots x_m$, мы хотели бы найти наиболее вероятную последовательность состояний, лежащую в основе модели, то есть
$$\mathrm{arg\,max}_{s \in S^m}p(s|x, w)$$

Упростив это выражение получим
$$\mathrm{arg\,max}_{s \in S^m} \sum_{j=1}^{m} w \cdot \phi(x, j, s_{j-1}, s_j)$$

Эта проблема имеет четкий интуитивный смысл. Каждый переход из состояния $s_{j-1}$ в состояние $s_j$ имеет соответствующую оценку
...
Инициализация для $s \in S$:
$$\pi[1, s] = w \cdot \phi(x, 1, s_0, s)$$
где $s_0$- особое "начальное" состояние.

Для $j = 2 \dots m, s = 1 \dots k$:
$$\pi[j, s] = \max_{s' \in S} [\pi[j-1, s_0] + w \cdot \phi(x, j, s_0, s)]$$

Таким образом, мы имеем
$$\max_{s_1 \dots s_m} \sum_{j=1}^{m} w \cdot \phi(x, j, s_{j-1}, s_j) = \max_s \pi[m, s]$$

И так мы можем восстановить путь приводящий к максимальной оценке.
$$L(w) = \sum_{i=1}^{n} \log(p(s^i|x^i;w))$$

Подбор параметров CRF

где $x^i, y^i$ примеры из тренировочного датасета.
