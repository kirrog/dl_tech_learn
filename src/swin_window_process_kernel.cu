...
at::Tensor
roll_and_window_partition_forward_cuda(
at::Tensor & input,
// at::Tensor & output,
const
int
...
dim3
grid(window_size, window_size, B * nH * nW);
// dim3
block((C + 31) / 32 * 32);
int
blocknum = best_block_dim(C);
dim3
block(blocknum);
...
output = torch::
    empty({B * nH * nW, window_size, window_size, C}, torch::dtype(torch::kFloat16).device(torch::kCUDA).requires_grad(
    true));
}
else {
    output = torch::empty({B * nH * nW, window_size, window_size, C}, torch::dtype(torch::kFloat32).device(
    torch::kCUDA).requires_grad(true));
}
...
// grad_in: [B * nH * nW, window_size, window_size, C]
            // grad_out: [B, H, W, C]
at::Tensor
roll_and_window_partition_backward_cuda(
at::Tensor & grad_in,
const
int
B,
...
grad_out = torch::
    empty({B, H, W, C}, torch::dtype(torch::kFloat16).device(torch::kCUDA).requires_grad(false));
}
else {
...
// input: [B * nH * nW, window_size, window_size, C]
          // output: [B, H, W, C]
at::Tensor
window_merge_and_roll_forward_cuda(
at::Tensor & input,
// at::Tensor & output,
const
int
...
if (input.scalar_type() == torch::kFloat16){
output = torch::
    empty({B, H, W, C}, torch::dtype(torch::kFloat16).device(torch::kCUDA).requires_grad(true));
}
else {
...
at::Tensor
window_merge_and_roll_backward_cuda(
at::Tensor & grad_in,
const
int
B,
...
grad_out = torch::
    ...
    grad_out = torch::empty({B * nH * nW, window_size, window_size, C}, torch::dtype(torch::kFloat32).device(
    torch::kCUDA).requires_grad(false));
}
